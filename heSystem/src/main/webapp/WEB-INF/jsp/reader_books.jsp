...
<div style="padding: 20px 550px 10px">
    <form method="post" action="reader_querybook_do.html" class="form-inline" id="searchform">
        <div class="input-group">
            <input type="text" placeholder="输入饮食名" class="form-control" id="search" name="searchWord"
                   class="form-control">
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${books}" var="book">
                <tr>
                    ...
                    <td><c:out value="${book.ingredients}"></c:out></td>
                    <td><c:out value="${book.population}"></c:out></td>
                    <td><c:out value="${book.cookingmethod}"></c:out></td>
                    ...
                    <c:if test="${not flag}">
                        <c:if test="${book.number>0}">
                            <td><a href="lendbook.html?bookId=<c:out value="${book.bookId}"></c:out>">
                                <button type="button" class="btn btn-primary btn-xs">收藏</button>
                            </a></td>
                        </c:if>
                        <c:if test="${book.number==0}">
                            <td>
